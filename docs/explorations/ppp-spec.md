...
- **v1 macOS concurrency limit:** Podman Machine permits only **one running VM at a time** (a podman-policy limit, not a hypervisor one — see ADR-0007). So on v1/macOS at most **one sandbox is *active* at a time**; multiple sandboxes may be created and coexist stopped. `ppp run`/start refuses a second active sandbox with a clear "stop the running one first" message. The multi-WireGuard machinery below is retained unchanged (correct for one active client, forward-compatible with concurrency); true concurrent per-sandbox microVMs are a v2 direction (drop below `podman machine` — Lima or direct krunkit/vfkit).
