...
- **IPC:** UDS socket at `$PPP_DATA/secret.sock`. The Go parent listens; the Python addon connects and sends `{"service": "anthropic", "sandbox": "ppp-red-bird"}`. The parent responds `{"key": "sk-ant-..."}`. If the secret is sandbox-scoped, the parent checks `ppp.<sandbox>.<service>` first, then falls back to `ppp.<service>` (global).
